...
class Cm_Mongo_Model_Resource_Collection_Abstract extends Cm_Mongo_Collection
{
    ...
    public function addFieldToFilter($field, $condition = NULL, $_condition = NULL)
    {
        switch (func_num_args()) {
            case 1:
                $query = $this->_getCondition($field);
                break;
            case 2:
                $query = $this->_getCondition($field, $condition);
                break;
            case 3:
            default:
                $query = $this->_getCondition($field, $condition, $_condition);
                break;
        }
    ...
    protected function _getCondition($fieldName, $condition = NULL, $_condition = NULL)
    {
        ...
        if (is_array($fieldName)) {
            ...
            foreach (explode('|', $fieldName) as $_fieldName) {
                switch (func_num_args()) {
                    case 2:
                        $query[] = $this->_getCondition($_fieldName, $condition);
                        break;
                    case 3:
                    default:
            ...
            switch (func_num_args()) {
                case 2:
                    $collection->addFieldToFilter($referenceField, $condition);
                    break;
                case 3:
                default:
